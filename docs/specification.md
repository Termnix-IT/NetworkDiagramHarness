...
- Mermaid CLI または Graphviz を使った画像出力
...
Mermaid、Graphviz DOT、Markdown、画像ファイルは生成物です。実構成を編集する場合も、画像や Markdown ではなく YAML を更新します。
...
| `export` | 単一 YAML を SVG/PNG/PDF に出力します。既定は Mermaid renderer です。 |
...
既定の画像出力には Mermaid CLI の `mmdc` が必要です。Graphviz renderer を使う場合は Graphviz の `dot` が必要です。
...
- draw.io 形式は未実装
- 自動レイアウトは Mermaid または Graphviz に依存する
