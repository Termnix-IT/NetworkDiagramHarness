...
YAML 入力の詳細は [schema.md](schema.md)、プロンプト運用は [prompt-workflow.md](prompt-workflow.md)、画像出力は [image-export-workflow.md](image-export-workflow.md)、掲載向け layout 出力は [layout-workflow.md](layout-workflow.md)、フォルダ構造は [folder-structure.md](folder-structure.md) を参照します。
...
- Mermaid CLI、Graphviz、drawsvg-style SVG、Pyvis-style HTML を使った出力
- 固定配置 template による掲載向け SVG 出力
...
- Mermaid CLI なしでの Mermaid 画像 export
- Graphviz なしでの Graphviz SVG/PNG/PDF 生成
...
Mermaid、Graphviz DOT、Markdown、SVG、PNG、PDF、HTML は生成物です。実構成を編集する場合も、画像や Markdown ではなく YAML を更新します。
...
| Graphviz DOT | `render --renderer graphviz` | Graphviz source の確認や外部利用 |
| drawsvg SVG | `render --renderer drawsvg` | 追加外部コマンドなしの静的 SVG |
| Pyvis HTML | `render --renderer pyvis` | インタラクティブ確認用 HTML |
...
| `export` | 単一 YAML を SVG/PNG/PDF/HTML に出力します。既定は Mermaid renderer です。 |
| `export-all` | ディレクトリ配下の `*.yml` をまとめて画像出力します。 |
| `export-layout` | `layout.profile` に従って固定配置 SVG を出力します。 |
...
## Graphviz Rendering Rules

Graphviz rendering は `renderers/graphviz.py` が担当します。

主な規則:

- `direction` は Graphviz の `rankdir` に反映する
- `zones` は `cluster` として出力する
- `node.type` ごとに `shape`, `fillcolor`, `color` を変える
- `style.profile` に応じて色を切り替える
- `node.rank`, `node.order`, `node.group` を layout hint として扱う
- `node.ip` と `node.role` は node label に表示する
- `connection.label` があればそれを優先する
- `label` がなければ `protocol / port / purpose` から接続ラベルを作る
- `connection.direction: inbound` は edge の向きを反転する
- `connection.direction: bidirectional` は `dir=both` として出力する

## drawsvg And Pyvis Rendering Rules

`drawsvg` renderer は `renderers/drawsvg.py` が担当します。追加外部コマンドなしで静的 SVG を出力します。

`pyvis` renderer は `renderers/pyvis.py` が担当します。`vis-network` 互換の HTML を出力し、ブラウザ上で node を動かして確認できます。
...
既定の画像出力には Mermaid CLI の `mmdc` が必要です。Graphviz renderer を使う場合は Graphviz の `dot` が必要です。`drawsvg` と `pyvis` renderer は追加外部コマンドなしで出力します。
...
Graphviz renderer を使う場合:

```powershell
network-diagram-harness export local/test-network.local.yml --renderer graphviz --output output/images/test-network.svg
```

Pyvis renderer を使う場合:

```powershell
network-diagram-harness export local/test-network.local.yml --renderer pyvis --output output/images/test-network.html
```
...
`mmdc` や `dot` がない環境でも `validate`, `render`, `preview` は利用可能です。
...
`home_lab` は Internet、Cloud、Edge、DMZ、Internal、VLAN/subnet 系の領域を固定位置に配置します。
...
- Graphviz 未導入時は `error: Graphviz dot executable was not found...` として表示する
- renderer と拡張子が合わない場合は unsupported format error として表示する
...
- Mermaid renderer の画像出力には Mermaid CLI が必要
- Graphviz renderer の画像出力には Graphviz が必要
- draw.io 形式は未実装
- 自動レイアウトは renderer ごとの実装に依存する
- 固定配置 SVG は現時点で `home_lab` profile のみ対応する
